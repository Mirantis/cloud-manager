...
func (s *Server) SetupRoutes() *gin.Engine {
	...
	apiProtected.Use(middleware.WriteAccessMiddleware())
	{
		...
		// Route53 hosted zone routes
			// EKS routes
			apiProtected.GET("/accounts/:accountId/eks-clusters", s.handler.ListEKSClustersByAccount)
			apiProtected.GET("/eks-clusters", s.handler.ListEKSClusters)
		apiProtected.GET("/route53-hosted-zones", s.handler.ListRoute53HostedZones)
		apiProtected.GET("/accounts/:accountId/route53-hosted-zones/:hostedZoneId/records", s.handler.ListRoute53Records)
